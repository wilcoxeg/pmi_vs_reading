...
cor.test(combined_df$freq, combined_df$surp)
cor.test(combined_df$freq, combined_df$pmi)

m_surp = lm(gaze_rt ~ surp + prev_surp + prev2_surp + freq + prev_freq + prev2_freq, data=combined_df)
vif(m_surp)

m_pmi = lm(gaze_rt ~ pmi + prev_pmi + prev2_pmi + freq + prev_freq + prev2_freq, data=combined_df)
vif(m_pmi)
...
ggsave("./images/orthogonalized_comparison_coeffs_wt.pdf", width = 5.5, height = 3.5)
...
```{r}
estimate_df = coeff_plotting_df %>%
  dplyr::select(lang, name, orthogonalized, surp, m) %>%
  filter(name == "frequency" & surp == T) %>%
  mutate(orthogonalized = if_else(orthogonalized == T, "orth", "non_orth")) %>%
  spread(orthogonalized, m) %>%
  mutate(diff = orth - non_orth) %>%
  mutate( percent_decrease = diff / orth)

mean(estimate_df$percent_decrease)
...
ggsave("./images/results_agg_all_langs.pdf", device = "pdf", width = 3.5, height = 3)
...
dll_agg_surp_df %>%
  ...
  ggplot(aes(x = target, y = mean, color = target)) + scale_x_discrete(labels =c("freq", "surp", "pmi", "orthogonalized\nsurp/pmi", "surp+freq", "pmi+freq", "orthogonalized\nsurp/pmi+freq")) + 
    geom_hline(yintercept=0, color="black", linetype="dashed", alpha =0.5) +
...
ggsave("./images/results_agg_gd.pdf", device = "pdf", width = 3, height = 4.5)
